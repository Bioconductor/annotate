...
Bioconductor's *AnnotationDbi* package and distributed as regular R
...
for specific organisms such as human, mouse, fly, or rat. *KEGG.db* and
*GO.db* are source specific libraries containing generic data for
various genomes.
...
the organism wide genome anotation packages is based upon some type of
...
*GO.db* library maintain the directed acyclic graph structure of the
...
complement the the GO.db package. These mappings are found within the
...
site ((http://www.bioconductor.org)). To illustrate their usages, we use
the library for Affymetrix HG_U95Av2 chip (*hgu95av2.db*) as an example
for platform specific data packages and the *GO.db* library for
...
Download libraries *hgu95av2.db* and *GO.db* with
BiocManager::install(). If the *reposTools* library has already been
installed/loaded, typing `install.packages2(library name)` installs the
library for both Unix and Windows.
...
the name of an mapping object proceeded by a question mark (e. g.
...
`hgu95av2GO` annotates probes on the HG`\-`{=latex}U95Av2 chip with ids
of the Gene Ontology terms the probes correspond to.

The name of an mapping object consists of package basename
(*hgu95av2.db*) and mapping name (*GO*) to avoid confusion when multiple
...
allow traverse between packages (Fig. 1). Using the example above, we
...
named *GO.db*. AS package *hgu95av2.db* and *GO.db* are linked by *GO*
ids, one can annotate probe id *738_at* with Gene Ontology terms by
linking data in the two packages using *GO* id as shown below:
