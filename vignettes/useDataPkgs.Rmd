...
author: 
  - name: "Marc Carlson, Jianhua Zhang"
...
for specific organisms such as human, mouse, fly, or rat. `r Biocpkg("KEGG.db")` and
`r Biocpkg("GO.db")` are source specific libraries containing generic data for
various genomes.
...
The `r Biocpkg("KEGG.db")` library contains mappings between ids such as Entrez Gene
...
All the annotation libraries can be obtained from http://www.bioconductor.org.
...
and the `r Biocpkg("GO.db")` library for non-platform specific data packages.  
We assume that [R](www.r-project.org) and Bioconductor's `r Biocpkg("Biobase")`
and `r Biocpkg("annotation")`.
